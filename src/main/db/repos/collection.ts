 ...
 * A row's identity.
 *
 * Derived rows have no id, so identity is the source plus what makes the grouping
 * unique. The UI keys selection and React children off this — and two queries answer
 * with it now, the page and the select-all, so it is written once. Two spellings of a
 * key would look like a selection that quietly matches nothing.
 */
function rowKey(
  source: string,
  itemId: number | null,
...
function toCollectionRow(row: JoinedRow): CollectionRow {
  ...
  return {
    key: rowKey(source, row.item_id, row.row_scryfall_id, row.finish),
    source,
    id: row.item_id,
...
/**
 * Every row the filters match, as keys and ids and nothing else.
 *
 * The list draws at most one page — 200 rows of a filtered set that is often far larger,
 * which the footer says out loud — so "select everything I am looking at" cannot be
 * answered from what the renderer holds. This answers it from the same `buildWhere` and
 * `FROM_ROWS` the page uses, so the two can never disagree about what matches.
 *
 * Ids come with the keys because a row that is not loaded has no `CollectionRow` on the
 * other side of the IPC, and every bulk action needs an id. Capped, and it says when it
 * capped: a selection nobody can see the end of is not worth an unbounded array.
 */
export function matchingRowKeys(
  filters: CollectionFilters,
  currency: Currency,
  cap = 20000
): { rows: { key: string; id: number | null }[]; truncated: boolean } {
  const where = buildWhere(filters, currency)
  const found = getDb().all(
    `SELECT r.source, r.item_id, r.scryfall_id AS row_scryfall_id, r.finish
     ${FROM_ROWS}
     WHERE ${where.sql}
     LIMIT ?`,
    [...where.params, cap + 1]
  ) as { source: string; item_id: number | null; row_scryfall_id: string; finish: string }[]
  const truncated = found.length > cap
  return {
    rows: found.slice(0, cap).map((row) => ({
      key: rowKey(row.source, row.item_id, row.row_scryfall_id, row.finish),
      id: row.source === 'collection' ? row.item_id : null
    })),
    truncated
  }
}
...
export function addToCollection(input: AddCardInput): number {
  const db = getDb()
  const now = nowIso()
  const scryfallId = input.scryfall_id
  db.run(
...
export function ownedCounts(scryfallIds: string[]): Map<string, number> {
  ...
  const rows = getDb().all(
    `SELECT ci.scryfall_id AS scryfall_id, COALESCE(SUM(ci.quantity), 0) AS total
       FROM collection_items ci
      WHERE ci.scryfall_id IN (${placeholders})
      GROUP BY ci.scryfall_id`,
    scryfallIds
